...
*/
    ...
    chrome.storage.local.get(["LHproject6TitleStorage"], function(result) {
        if (result.LHproject6TitleStorage === undefined) {
            chrome.storage.local.set({ "LHproject6TitleStorage": "Project #6" });
            project6Title.value = "Project #6";
        } else {
            project6Title.value = result.LHproject6TitleStorage;
        }

        project6Title.addEventListener("keydown", function(event) {
            if (event.key === "Enter") {
                project6Title.blur();
            }
        });

        project6Title.addEventListener("blur", function(event) {
            const project6CurrentTitle = event.target.value;
            project6Title.value = project6CurrentTitle;
            chrome.storage.local.set({ "LHproject6TitleStorage": project6CurrentTitle });
        });
    });

    // Add more sources
    const addMoreSources = document.getElementById("add__more__sources");
    const deleteProject6URL = document.getElementById("deleteButton");
    deleteProject6URL.style.display = "none";

    const clearProject6URL = document.getElementById("clearButton");
    const changeBackground = document.getElementById("project__header");


    const toggleProject6 = document.getElementById("toggleProject6");

    let Project6URL, Project6Article, Project6Author, Project6PublishedDate, Project6Summary;
    ...
    */
    ...
    function updateStorageInformation () {
        chrome.storage.local.get(["selectedProject", "LHProject6URL", "LHProject6Article", "LHProject6Author", "LHProject6PublishedDate", "LHProject6Summary", "LHProject6"], function(result) {
            console.log("Result:", result);
            
            Project6URL = result.LHProject6URL || [];
            Project6Article = result.LHProject6Article || [];
            Project6Author = result.LHProject6Author || [];
            Project6PublishedDate = result.LHProject6PublishedDate || [];
            Project6Summary = result.LHProject6Summary || [];
        ...
        function sourceDisplay() {
            ...
            console.log("Project6URLlength:", Project6URL.length);
            if (Project6URL.length > 0) {
                addMoreSources.style.display = "none";
                for (let i = 0; i < Project6URL.length; i++) {
                    ...
                    articleTextSummary.textContent = Project6Summary[i];
                    ...
                    articleTitle.textContent = Project6Article[i];
                    ...
                    articleURL.textContent = Project6URL[i];
                    ...
                    chrome.storage.local.get(["SavedType"], function(result) {
                        if (Project6URL[i].length > 32) {
                            const shortenedURL =  Project6URL[i].slice(0, 32) + '...';
                        
                        if (result.SavedType === "normal") {
                            APAformat.innerHTML = `${Project6Author[i]}. (${Project6PublishedDate[i]}). <i>${Project6Article[i]}. </i>${shortenedURL}\n\n`;
                        } else if (result.SavedType === "YouTube") {
                            APAformat.innerHTML = `${Project6Author[i]}. (${Project6PublishedDate[i]}). <i>${Project6Article[i]}. </i>[Video]. YouTube. ${shortenedURL}\n\n`;
                        }
                } else {
                    if (result.SavedType === "normal") {
                        APAformat.innerHTML = `${Project6Author[i]}. (${Project6PublishedDate[i]}). <i>${Project6Article[i]}. </i>${Project6URL[i]}\n\n`;
                    } else if (result.SavedType === "YouTube") {
                        APAformat.innerHTML = `${Project6Author[i]}. (${Project6PublishedDate[i]}). <i>${Project6Article[i]}. </i>[Video]. YouTube. ${shortenedURL}\n\n`;
                    }
                }
                ...
                articleSummary.addEventListener("click", function() {
                    ...
                    } else if (articleTextSummary.style.display === "block" && !toggleProject6.checked) { 
                        APAformat.style.display = "none";
                ...
                toggleProject6.addEventListener("change", function() {
                    
                    if (!toggleProject6.checked) {
                    ...
                    } else if (toggleProject6.checked) {
                        normalFormatElements.forEach(element => {
                            element.style.display = "none";
                        });
                        APAFormatElements.forEach(element => {
                ...
                toggleProject6.dispatchEvent(new Event("change"));
            }
        } else {
            addMoreSources.style.display = "block";
        }
...
//const selectedButtons = document.querySelectorAll(".selected__button") || null;
        ...
        cancelButton.addEventListener("click", function() {
            clearProject6URL.style.display = "block";
            deleteProject6URL.style.display = "none";
            cancelButton.style.display = "none";
            changeBackground.style.backgroundColor = "#f0f8ff";
            project6Title.style.display = "block";
            /*
            ...
            for (let i = 0; i < Project6URL.length; i++) {
                document.getElementById("sources__button" + i).addEventListener("click", function(event) {
                    clearTimeout(clickTimeout);
                        ...
                        clearProject6URL.style.display = "none";
                        deleteProject6URL.style.display = "block";

                        cancelButton.style.display = "block";
                        changeBackground.style.backgroundColor = "#a293ad";
                        project6Title.style.display = "none";
                        ...
                        for (let i = 0; i < Project6URL.length; i++) {
                            if (selectedButton[i].style.display === "block") {
                                selectedURL.push(Project6URL[i]);
                                selectedArticle.push(Project6Article[i]);
                                selectedAuthor.push(Project6Author[i]);
                                selectedPublishedDate.push(Project6PublishedDate[i]);
                                selectedSummary.push(Project6Summary[i]);
                            }
                        }
    ...
    clearProject6URL.addEventListener("click", function() {
        if (Project6URL.length > 0) {
            ...
            if (userConfirmed) {
                chrome.storage.local.set({
                    ...
                    LHProject6Article: [],
                    LHProject6Author: [],
                    LHProject6PublishedDate: [],
                    LHProject6Summary: [],
                    LHproject6TitleStorage: "Project #6"
                }, function() {
    ...
    deleteProject6URL.addEventListener("click", function() {
        if (Project6URL.length > 0) {
            for (let i = 0; i < Project6URL.length; i++) {
                if (document.getElementById("sources__button" + i).style.color === "orange") {
                    Project6URL.splice(i, 1);
                    Project6Article.splice(i, 1);
                    Project6Author.splice(i, 1);
                    Project6PublishedDate.splice(i, 1);
                    Project6Summary.splice(i, 1);

                    chrome.storage.local.set({ "LHProject6URL": Project6URL });
                    chrome.storage.local.set({ "LHProject6Article": Project6Article });
                    chrome.storage.local.set({ "LHProject6Author": Project6Author });
                    chrome.storage.local.set({ "LHProject6PublishedDate": Project6PublishedDate });
                    chrome.storage.local.set({ "LHProject6Summary": Project6Summary });
    ...
    chrome.storage.local.get(["LHProject6URL", "LHProject6Article", "LHProject6Author", "LHProject6PublishedDate", "LHProject6Summary"], function(result) {
        ...
        function exportingBasics() {
            ...
            if (Project6URL.length > 0) {
                if (toggleProject6.checked) {
                    subject = project6Title.value + " [APA Citations]\n\n";
                } else if (!toggleProject6.checked) {
                    subject = project6Title.value + " [Sources]\n\n";
                }
                ...
                if (selectedURL) {
                    ...
                    selectedURL = [];
                        ...
                        entries = Project6URL.map((url, index) => ({
                            author: Project6Author[index],
                            publishedDate: Project6PublishedDate[index],
                            article: Project6Article[index],
                            summary: Project6Summary[index],
                            url: url,
                            isYouTube: url.includes("https://youtube.com/watch?")
                        }));
